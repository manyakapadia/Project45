...
function homeScreen(){
  ...
  playButton.position(width / 2 - 20, height / 2 + 250);
...
function scene1(){
  ...
  // //scale is being applied to all
  // //scale(0.3);
  // image(goblet7Img, windowWidth/2, windowHeight/2);
  // image(locketImg, windowWidth/2, windowHeight/2);
  ...
  if(mousePressedOver(goblet7)){
    ...
    fill("white");
    textSize(30);
    text("Congratulations! You have succeessfully found and destroyed the cup! ", 50,50);
  }
  ...
  if(mousePressedOver(ring)){
    ...
    fill("white");
    textSize(30);
    text("Congratulations! You have succeessfully found and destroyed the ring! You are one step closer to defeating Voldemort", 50,50);
  }

  if(ringFound === true && goblet7Found === true){
    scene2();
    scene1.hide();
  }
}
